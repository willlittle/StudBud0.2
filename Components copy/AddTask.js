...
export default function FormPropsTextFields({submit}) {
  ...
  const handleSub = (title, description, subject,taskType,priority,hr,min,selectedDate) =>{
    ...
    let minute = 0
    let hrT = /^\d+$/.test(hr)
    let minT = /^\d+$/.test(min)//taken fromhttps://stackoverflow.com/questions/1779013/check-if-string-contains-only-digits , testing if digits
    if (hrT && minT){
      hour = Number(hr)
      minute = Number(min)
    ...
    submit(title, description, subject,taskType,priority,hour,minute,selectedDate)
    setTitle('') 
    setDescription('')
    setSubject('')
    setTaskType('')
